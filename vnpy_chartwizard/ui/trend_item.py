...
class TrendLineItem(ChartItem):
    ...
    def __init__(self, manager: BarManager) -> None:
        ...
        # Computed line segments: (x0, y0, x1, y1, pen).
        self._segments: list[tuple[float, float, float, float, QtGui.QPen]] = []
    ...
    def _recompute(self) -> None:
        ...
        if self.show_trend:
            ...
            if len(highs) >= 2:
                ...
                if x1 != x0:
                    ...
                    self._segments.append((x0, y0, last_ix, y_end, self._res_pen))
                    ys += [y0, y_end]
            ...
            if len(lows) >= 2:
                ...
                if x1 != x0:
                    slope = (y1 - y0) / (x1 - x0)
                    y_end = y1 + slope * (last_ix - x1)
                    self._segments.append((x0, y0, last_ix, y_end, self._sup_pen))
                    ys += [y0, y_end]
        ...
        if self.show_channel:
            ...
            self._segments.append((x_left, mid_left, x_right, mid_right, self._chan_mid_pen))
            self._segments.append((x_left, mid_left + band, x_right, mid_right + band, self._chan_band_pen))
            self._segments.append((x_left, mid_left - band, x_right, mid_right - band, self._chan_band_pen))
    ...
    def paint(self, painter: QtGui.QPainter, opt, w) -> None:  # type: ignore[override]
        if not self._segments:
            return
        for x0, y0, x1, y1, pen in self._segments:
            painter.setPen(pen)
